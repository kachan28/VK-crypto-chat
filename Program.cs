...
namespace VKMESTEST 
{
    class Program
    {
        private static void Auth(VkApi api)
        {
        ...
        private static void Two_Fact_Auth(VkApi api)
        {
            bool pr = false;
            while (!pr)
            {
        ...
        private static void One_Fact_Auth(VkApi api)
        {
            bool pr = false;
            while (!pr)
            {
...
//            {
        ...
        {
            Console.WriteLine("Если какие то из параметров отсутствуют у пользователя, то просто пропустите их ввод, нажав клавишу Enter.");
            TerminOutput();
